...
Auto-Encoders are only able to reproduce the elements of the training set but not to interpolate among them; Kingma and Welling \cite{vae} introduced the Variational Auto-Encoder (VAE) class of algorithms, or more correctly "Auto-Encoding Variational Bayes" methods as they are more related to Variational Bayesian methods then Auto-Encoders.
...
The loss functions of AEs is a ``distance'' between the input element and the one generated in output; in VAEs there is another term, typically a Kullback-Leibler (KL)\cite{kl} divergence, that measures the distance between the encoding PDF and a reference PDF, usually a Normal distribution.
